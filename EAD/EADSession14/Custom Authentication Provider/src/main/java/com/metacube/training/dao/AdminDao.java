...
public class AdminDao {
	...
	/**
	 * Returns Roles of Employees
	 * 
	 * @param email
	 * @return
	 */
	public List<UserRoles> getRolesOfEmployee(String email) {
		List<UserRoles> roles = new ArrayList<UserRoles>();
		try {
			connection = ConnectionToDatabase.getConnection();
			String query = Queries.SEARCH_ROLES;
			PreparedStatement statement = connection.prepareStatement(query);
			statement.setString(1, email);
			ResultSet resultSet = statement.executeQuery();
			while (resultSet.next()) {
				roles.add(new UserRoles((String) resultSet.getString(1)));
			}
		} catch (SQLException ex) {
			ex.printStackTrace();
		} catch (Exception ex) {
