...
public final class Poly {
	...
	public Poly(int inputPolyArray[][]) throws PolynomialException {
		...
		if (inputPolyArray[0].length != 2) {
			throw new PolynomialException(
					"Array Width can't be more or less than 2");
		}
		...
		lengthOfInputArray = Poly.mergeArrayValues(inputPolyArray,
				inputPolyArray.length);
		/*
	...
	public double evaluate(double valueOfVariable) {
		...
		for (int i = 0; i < polyInput.length; i++) {
			evaluation += polyInput[i][0]
					* Math.pow(valueOfVariable, polyInput[i][1]);
		}
	...
	public static int[][] addPoly(Poly polynomial1, Poly polynomial2) {
		int addPoly[][] = new int[polynomial1.polyInput.length
				+ polynomial2.polyInput.length][2];
		int lengthOfAddPoly = 0;
		int finalAddPoly[][];

		/*
		 * Addition of the Coefficients of Same powers and for different powers
		 * just Inserting them in addPoly[][]
		 */
		int indexOfPoly1 = 0;
		int indexOfPoly2 = 0;
	...
	public static int[][] multiplyPoly(Poly polynomial1, Poly polynomial2) {
		int multiplyPoly[][] = new int[polynomial1.polyInput.length
				* polynomial2.polyInput.length][2];
		...
		/*
		 * Multiplying Every Coefficient and Powers of Polynomials with each
		 * other
		 */
		for (int i = 0; i < polynomial1.polyInput.length; i++) {
			for (int j = 0; j < polynomial2.polyInput.length; j++) {
				multiplyPoly[lengthOfMultiplyPoly][0] = polynomial1.polyInput[i][0]
						* polynomial2.polyInput[j][0];
				multiplyPoly[lengthOfMultiplyPoly][1] = polynomial1.polyInput[i][1]
						+ polynomial2.polyInput[j][1];
				lengthOfMultiplyPoly++;
			}
		}
		Poly.sortArray(multiplyPoly, lengthOfMultiplyPoly);
		finalLengthOfMultiplyPoly = Poly.mergeArrayValues(multiplyPoly,
				lengthOfMultiplyPoly);
