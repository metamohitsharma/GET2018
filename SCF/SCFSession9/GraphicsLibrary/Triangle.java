...
public class Triangle implements Shape {
	 ...
	 * satisfies where (x,y) is the coordinate of Point, (xO,yO) is the origin, m1
	 * is slopeA, m2 is slopeB then the Point lies inside the Triangle otherwise not
	 */
	@Override
	...
	/**
	 * In a Triangle there are three sides and three coordinates We are taking sideA
	 * as the Leftmost side of Base side We are taking sideB as the Rightmost side
	 * of Base side The intersection of sideA and sideB is the PointA The
	 * intersection of sideB and Base side is the Point B
	 * 
	 * @throws ScreenException
	 */
	private void computeOtherParameters() throws ScreenException {
		double differenceOfBase = Math.sqrt(Math.pow(sideA, 2) - Math.pow(height, 2));
		double otherDifferenceOfBase = base - differenceOfBase;
		sideB = Math.sqrt(Math.pow(height, 2) + Math.pow(otherDifferenceOfBase, 2));
		pointA = new Point(origin.getxPoint() + differenceOfBase, origin.getyPoint() + height);
